...
from mlir.ir import register_attribute_builder, Attribute, IntegerType, IntegerAttr
...
class AllReduceOperation(IntEnum):
    ...
    def __str__(self):
        ...
        if self is AllReduceOperation.AND:
            return "and"
        if self is AllReduceOperation.MAX:
            return "max"
        if self is AllReduceOperation.MIN:
            return "min"
...
def _gpu_spgemmworkestimationorcomputekindattr(x, context):
    return Attribute.parse(
        f"#gpu<spgemm_work_estimation_or_compute_kind {str(x)}>", context=context
    )


@register_attribute_builder("GPU_TransposeModeAttr")
def _gpu_transposemodeattr(x, context):
    return Attribute.parse(f"#gpu<mat_transpose_mode {str(x)}>", context=context)


@register_attribute_builder("MMAElementWiseAttr")
def _mmaelementwiseattr(x, context):
    return Attribute.parse(f"#gpu<mma_element_wise {str(x)}>", context=context)
