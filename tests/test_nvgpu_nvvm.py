...
def test_transform_mma_sync_matmul_f16_f16_accum(ctx: MLIRContext, capfd):
    ...
    correct = (
        correct.replace(
            "vector.splat %2 : vector<4x2xf16>",
            "vector.broadcast %2 : f16 to vector<4x2xf16>",
        )
        .replace(
            "vector.splat %23 : vector<2x2xf16>",
            "vector.broadcast %23 : f16 to vector<2x2xf16>",
        )
        .replace(
            "vector.splat %32 : vector<2x2xf16>",
            "vector.broadcast %32 : f16 to vector<2x2xf16>",
        )
    )
