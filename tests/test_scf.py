...
def test_for_bare(ctx: MLIRContext):
    ...
    assert isinstance(i1, Scalar) and repr(i1) == "Scalar(%0#0, f32)"
    assert isinstance(i2, Scalar) and repr(i2) == "Scalar(%0#1, f32)"
...
def test_scf_canonicalizer(ctx: MLIRContext):
    ...
    def foo():
        ...
        assert isinstance(i1, Scalar) and repr(i1) == "Scalar(%0, f32)"
...
def test_if_replace_yield_4(ctx: MLIRContext):
    ...
    def iffoo():
        one = constant(1.0)
        two = constant(2.0)
        if res := stack_if(one < two, (f32_t, f32_t)):
            three = constant(3.0)
            yield three, three
        else:
            four = constant(4.0)
...
def test_if_with_elif_elif_explicit_yields_results(ctx: MLIRContext):
    ...
    def iffoo():
        ...
        if res := stack_if(one < two, (f32_t, f32_t)):
            five = constant(5.0)
            yield five, five
        elif res1 := stack_if(two < three, (f32_t, f32_t)):
            six = constant(6.0)
            yield six, six
        elif res2 := stack_if(three < four, (f32_t, f32_t)):
            seven = constant(7.0)
            yield seven, seven
        else:
            eight = constant(8.0)
