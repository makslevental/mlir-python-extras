...
from .util import add_file_to_sources_txt_file
...
def generate_trampolines(
    ...
    if dst_path is None:
        dst_path = Path(dialects.__path__[0])
    ...
    if generated := generate_dialect_trampolines_from_module(modu, skips):
        ...
        add_file_to_sources_txt_file(dst_path)
...
def generate_all_upstream_trampolines():
    ...
    mlir_mod_path = Path(dialects.__path__[0])
    if not mlir_mod_path.exists():
        os.makedirs(mlir_mod_path)

    for mod in pkgutil.iter_modules(mlir_dialects.__path__):
