...
def find_ancestor_block_in_region(block: Block):
    ...
    while curr_block and curr_block.region != block.region:
        ...
        if not parent_op or not parent_op.block:
            return None
        curr_block = parent_op.block
    return curr_block


def find_ancestor_op_in_block(op: Operation, block: Block):
    curr_op = op
    while curr_op and curr_op.block != block:
